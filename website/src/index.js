...
async function getStoreOffers(token, entitlement, puuid, region) {
	...
	if (offset > 0) {
		console.log(time);
		let fix = Math.abs(7 - offset);
		console.log(fix);
		if (fix + time.getHours() > 24) {
			time.setDate(time.getDate() + 1);
			time.setHours(0);
		}
		time.setHours(fix + time.getHours());
	} else {
		let fix = Math.abs(7 + offset);
		if (fix + time.getHours() > 24) {
			time.setDate(time.getDate() + 1);
	...
	if (localStorage.getItem("LastStoreData") != null) {
		let LastStoreData = localStorage.getItem("LastStoreData");
		console.log("Using Local Storage");
		return JSON.parse(LastStoreData);
	} else {
